	...
	public void validate(Object target, Errors errors) {
		...
		if(emp.getLastName() ==null ||
			...
			errors.rejectValue("lastName",
							"required",new Object[] {"lastName"},"");
			}
		
		ValidationUtils
		.rejectIfEmptyOrWhitespace(errors,"jobId", "required","jobid 필수입력");
		//이메일형식
		
		if(emp.getEmail() ==null ||
				emp.getEmail().trim().isEmpty())	{
			errors.rejectValue("email",
							"required", new Object[] {"email"},"");
			}else {
				Matcher matcher=
							pattern.matcher(emp.getEmail());
				if(! matcher.matches()) {
					errors.rejectValue("email","bad","email 형식이 아닙니다." );
				}
			}
	}
